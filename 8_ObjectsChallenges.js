...
function objEntries(obj){
  let newObjEntries = [];
  for (let keys in obj){
    newObjEntries.push([keys, obj[keys]]);
  } 
  return newObjEntries; 
}
...
function pluck (arrOfObjs, str){
  for (let index of arrOfObjs){
...
let finalStr = "";
  for (let keys in obj){
    finalStr = finalStr + `${keys} = ${obj[keys]}, `
  } 
  let slice =  finalStr.slice(0, (finalStr.length - 2));
  return `"${slice}"`
}
